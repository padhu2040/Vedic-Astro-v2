...
def get_house_strength_analysis(house_num, score, lang="English"):
    topics = {
        ...
        7: "Marriage & Partnerships", 8: "Transformation & Hidden Wealth", 
        9: "Luck, Wisdom & Fortune", 10: "Career & Executive Authority", 
        11: "Major Gains & Networks", 12: "Spiritual Retreats & Expenditures"
    }
    t_topics = {
    ...
    if score >= 30:
        if lang == "Tamil": return f"**மிகவும் வலிமையான பாவம்:** இந்த {house_num}-ஆம் பாவம் ({t_topics[house_num]}) இயற்கையாகவே அதிக சக்தி பெற்றுள்ளது. இந்தத் துறையில் சிறிய முயற்சிகள் கூட மாபெரும் வெற்றியைத் தரும்."
        return f"**Power Zone:** The {house_num}th House ({topics[house_num]}) is exceptionally strong. Energy flows effortlessly here. You possess a built-in advantage in this area of life."
    elif score < 25:
        if lang == "Tamil": return f"**கவனம் தேவைப்படும் பாவம்:** இந்த {house_num}-ஆம் பாவம் ({t_topics[house_num]}) சற்று பலவீனமாக உள்ளது. இங்கு வெற்றி பெற மற்றவர்களை விட அதிக உழைப்பும், திட்டமிடலும் தேவை."
        return f"**Challenge Zone:** The {house_num}th House ({topics[house_num]}) requires highly conscious, strategic effort. This area does not yield easy luck; you must actively manage it."
    else:
        ...
        return f"**Balanced Zone:** The {house_num}th House ({topics[house_num]}) is stable. It operates strictly on the rule of cause and effect. Input equals output."
...
def analyze_karmic_axis(p_pos, lagna_rasi, lang="English"):
    ...
    if lang == "Tamil":
        ...
        res.append(f"**Rahu's Ambition ({rahu_h}th House):** Rahu creates a relentless, almost obsessive hunger for growth in this house. You are meant to take massive, unconventional leaps here.")
        res.append(f"**Ketu's Mastery & Detachment ({ketu_h}th House):** Ketu signifies deep past-life mastery. You possess effortless, intuitive genius in this area, but you may simultaneously feel a profound sense of 'been there, done that' detachment. Use Ketu's analytical skills without abandoning the house's duties.")
    return res
...
def scan_yogas(p_pos, lagna_rasi, lang="English"):
    ...
    if p_pos.get("Sun") == p_pos.get("Mercury"):
        ...
        else: yogas.append({"Name": "Budhaditya Yoga", "Type": "Intellect & Commerce", "Description": f"The Sun and Mercury are conjunct in your {p_houses.get('Sun')}th House, forming a highly analytical business mind. **Impact:** It enhances executive communication and creates wealth potential through advisory or trade. You likely noticed this mental edge emerging early in your education."})
    ...
    if "Jupiter" in p_pos and "Moon" in p_pos:
        ...
        if jup_from_moon in [1, 4, 7, 10]:
            ...
            else: yogas.append({"Name": "Gajakesari Yoga", "Type": "Fame & Institutional Protection", "Description": "Jupiter is placed in a foundational angle from your Natal Moon. **Impact:** It grants a noble reputation and divine protection against crises. This yoga acts as a lifetime safety net, resolving major issues at the 11th hour."})
    ...
    if p_pos.get(lord_9) == p_pos.get(lord_10) and lord_9 != lord_10:
        if lang == "Tamil": yogas.append({"Name": "தர்ம கர்மாதிபதி யோகம்", "Type": "உயர்ந்த தொழில் அந்தஸ்து", "Description": "உங்களின் 9-ஆம் அதிபதியும் 10-ஆம் அதிபதியும் இணைந்திருப்பதால் இந்த யோகம் அமைகிறது. இது தொழில் ரீதியான மிக உயர்ந்த ராஜ யோகமாகும். நீங்கள் தொட்டதெல்லாம் துலங்கும்."})
...
def analyze_education(p_pos, lagna_rasi, lang="English"):
    ...
    if lang == "Tamil":
        ...
        if mercury_dig in ["Exalted", "Own"]: analysis.append("Because Mercury (planet of logic) is highly dignified, your capacity to process, structure, and deploy complex information is elite. You have a distinct, highly analytical advantage in technical, financial, or heavily communicative environments.")
...
def analyze_love_marriage(d1_lagna, d9_lagna, p_d9, p_d1, lang="English"):
    ...
    traits = {
        ...
        "Saturn": "unshakeable duty, absolute loyalty through hard times, and extreme patience"
    }
    t_traits = {
    ...
    if lang == "Tamil":
        ...
        analysis.append(f"The 7th House of your Navamsa (D9) reveals your ultimate spousal archetype. It is ruled by **{d9_7th_lord}**. To achieve a permanently successful marriage, your partner must fundamentally embody {d9_7th_lord}'s mature traits. Specifically, your union must be actively built upon **{traits[d9_7th_lord]}**.")
    return analysis
...
def analyze_health(p_pos, lagna_rasi, lang="English"):
    ...
    if lang == "Tamil":
        ...
        analysis.append(f"**கவனம் தேவை:** ஆரோக்கியத்தை குறிக்கும் 6-ஆம் அதிபதி **{t_p[lord_6]}** ஆவார். இந்த கிரகத்துடன் தொடர்புடைய உடல் பாகங்களில் நீங்கள் கூடுதல் தடுப்பு நடவடிக்கைகளை மேற்கொள்ள வேண்டும்.")
    else:
        ...
        elif ll_dig == "Neecha": analysis.append(f"Your Ascendant Lord ({lagna_lord}) is weak by sign placement. Your physical energy is finite and must be carefully budgeted. Strict sleep and dietary discipline are mandatory.")
        else: analysis.append(f"Your Ascendant Lord ({lagna_lord}) is in a neutral state. Your physical resilience is perfectly average and will act as a direct mirror to your lifestyle choices.")
        analysis.append(f"**Vulnerabilities:** The 6th House of acute health is ruled by **{lord_6}**. You must proactively monitor the physiological systems associated with {lord_6} (e.g., Mercury = nervous system/gut, Mars = inflammation/blood) to prevent chronic issues.")
    return analysis
...
def generate_annual_forecast(moon_rasi, sav_scores, f_year, age, lang="English"):
    ...
    fc = {}
    if lang == "Tamil":
        if sat_dist in [3, 6, 11]: fc['தொழில் மற்றும் சவால்கள் (Saturn)'] = ("மிகப்பெரிய வளர்ச்சி நிலை. உங்களின் கடந்த கால கடின உழைப்பிற்கு மிகப்பெரிய அங்கீகாரம், பதவி உயர்வு மற்றும் எதிரிகளை வீழ்த்தும் வெற்றி இந்த ஆண்டு கிடைக்கும்.", "சனிக்கிழமைகளில் நல்லெண்ணெய் தீபம் ஏற்றவும்.")
        elif sat_dist in [1, 2, 12]: fc['தொழில் மற்றும் சவால்கள் (Saturn)'] = ("ஏழரைச் சனி காலம். பொறுமை மிக மிக அவசியம். தொழில் மற்றும் குடும்பத்தில் உங்கள் பொறுப்புகள் பலமடங்கு கூடும். அவசர முடிவுகளைத் தவிர்க்கவும்.", "தினமும் ஹனுமான் சாலிசா படிக்கவும்.")
        ...
        if jup_dist in [2, 5, 7, 9, 11]: fc['பொருளாதாரம் மற்றும் குடும்பம் (Jupiter)'] = ("பிரமாண்டமான பணவரவு மற்றும் அதிர்ஷ்டம். திருமணம், குழந்தை பாக்கியம் மற்றும் சொத்து சேர்க்கை போன்ற சுப காரியங்கள் தடையின்றி நடைபெறும்.", "வியாழக்கிழமைகளில் மஞ்சள் நிற உணவுகளை தானம் செய்யவும்.")
        ...
        if sat_dist in [3, 6, 11]: fc['Career & Ambition (Saturn)'] = ("**EXCELLENT GROWTH PHASE.** Saturn is transiting a highly favorable growth house. Expect a major, structural promotion, deep recognition for past hard work, and a breakthrough victory over professional obstacles.", "Light a lamp with sesame oil on Saturdays.")
        elif sat_dist in [1, 2, 12]: fc['Career & Ambition (Saturn)'] = ("**SADE SATI PHASE.** You are under immense cosmic pressure to mature. You may feel professionally undervalued or heavily burdened. This is a time for extreme patience and strictly avoiding ego clashes.", "Chant Hanuman Chalisa daily.")
        else: fc['Career & Ambition (Saturn)'] = ("**STEADY PROGRESS.** There are no extreme highs or lows indicated by Saturn. This is a highly productive, 'heads-down' year. Use it to clear pending projects and build quiet momentum.", "Keep your workspace completely decluttered.")

        if jup_dist in [2, 5, 7, 9, 11]: fc['Wealth & Family Expansion (Jupiter)'] = ("**HIGH LUCK & INFLOW.** Jupiter actively blesses your chart this year. This is a phenomenal window for financial investments, purchasing property, or experiencing joyous family expansions (marriage/births).", "Donate yellow food (bananas/dal) on Thursdays.")
        else: fc['Wealth & Family Expansion (Jupiter)'] = ("**STABLE CONSOLIDATION.** Jupiter’s placement advises extreme financial caution. Strictly avoid high-risk speculation or gambling this year. Focus purely on savings and protecting your current assets.", "Keep a small turmeric stick in your wallet.")
    return fc
    ...
    for (trp, np), dates in active_conjunctions.items():
        ...
        if lang == "Tamil":
            ...
            if trp == "Saturn":
                ...
                elif np == "Rahu": meaning = "கர்ம வினைகளின் அழுத்தம் கூடும். தியானம் அவசியம்."
                elif np == "Ketu": meaning = "பழைய விஷயங்கள் முடிவுக்கு வந்து புதிய பாதை திறக்கும்."
            ...
            if trp == "Saturn":
                ...
                elif np == "Rahu": meaning = "Intense karmic pressure. Past actions catch up. Stay highly disciplined."
                elif np == "Ketu": meaning = "Old structures break down to force spiritual alignment. Let go of what no longer serves you."
