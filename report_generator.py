...
def get_south_indian_chart_html(p_pos, lagna_rasi, title="Rasi Chart", lang="English"):
    ...
    This matches your original preferred aesthetic with crisp borders and teal tags.
    """
    ...
    def get_box(rasi_num):
        # Styling the planet tags to match your screenshot (light teal background, bold teal text)
        ...
        return f"<div style='border: 1px solid #d3d3d3; height: 90px; padding: 6px; background: #ffffff; display: flex; flex-wrap: wrap; align-content: flex-start;'>{planets}</div>"

    html = f"""
    <div style="max-width: 450px; margin: 0 auto; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;">
        <div style="text-align: center; margin-bottom: 15px; font-size: 18px; font-weight: 700; color: #2c3e50;">{title}</div>
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 0; border: 1px solid #d3d3d3;">
            {get_box(12)} {get_box(1)} {get_box(2)} {get_box(3)}
            
            {get_box(11)} 
            <div style="grid-column: span 2; grid-row: span 2; display: flex; align-items: center; justify-content: center; background: #fafafa; font-size: 14px; font-weight: 500; color: #bdc3c7; border: 1px solid #d3d3d3;">{title}</div> 
            {get_box(4)}
...
class ExecutivePDF(FPDF):
    ...
    def chapter_title(self, title):
        # Section Header Styling (Teal line below text)
...
def clean_text(text):
    """Strips HTML tags and Streamlit Markdown formatting so FPDF renders it safely."""
    if not isinstance(text, str):
        return ""
    text = re.sub(r'<[^>]+>', '', text) # Remove HTML tags (e.g., <b>, <br>)
    text = re.sub(r'\*\*(.*?)\*\*', r'\1', text) # Remove bold markdown
    text = re.sub(r'_(.*?)_', r'\1', text) # Remove italic markdown
    text = re.sub(r'#(.*)', '', text) # Remove Hash Headers
    return text.strip()
