...
from astro_engine import ZODIAC_TA, ZODIAC, t_p
...
def get_south_indian_chart_html(p_pos, lagna_rasi, title, lang="English"):
    ...
    for p, r in p_pos.items():
        if p == "Lagna": continue # Skip adding Lagna text again
        # FIX: Rely exclusively on the robust t_p dictionary
        name = t_p.get(p, p[:2]) if lang == "Tamil" else p[:2] 
