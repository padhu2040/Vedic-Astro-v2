...
# --- SETUP CLOUD DATABASE (single definition) ---
@st.cache_resource
def init_connection():
    try:
...
# --- CLOUD DATABASE HELPER (single definition) ---
...
# Single definition of get_utc_offset (handles both date and datetime)
def get_utc_offset(tz_str, date_obj):
    try:
        tz = pytz.timezone(tz_str)
        if not isinstance(date_obj, datetime):
            date_obj = datetime.combine(date_obj, datetime(2000, 1, 1, 12, 0).time())
...
def get_navamsa_chart(lon):
    rasi_num = int(lon / 30) + 1
    pada = int((lon % 30) / NAV_SIZE) + 1
    if rasi_num in [1, 5, 9]:
        start = 1
    elif rasi_num in [2, 6, 10]:
        start = 10
    elif rasi_num in [3, 7, 11]:
        start = 7
    else:
        start = 4
...
def determine_house(planet_lon, cusps):
    ...
    for i in range(12):
        ...
        upper = cusps[(i + 1) % 12]
        if lower < upper:
            if lower <= p_lon < upper:
                return i + 1
        else:
            if p_lon >= lower or p_lon < upper:
                return i + 1
    return 1
...
def scan_yogas(p_pos, lagna_rasi, lang="English"):
    ...
    def house_of(planet):
        ...
        if r is None:
            return None
        return (r - lagna_rasi) % 12 + 1

    p_houses = {p: house_of(p) for p in p_pos if p != "Lagna" and house_of(p) is not None}

    if p_pos.get("Sun") == p_pos.get("Mercury"):
        if lang == "Tamil":
            yogas.append({
                "Name": "புதாதித்ய யோகம் (Budhaditya Yoga)",
                "Type": "அறிவு மற்றும் வணிகம்",
                "Description": f"சூரியனும் புதனும் உங்கள் {p_houses.get('Sun')}-ஆம் வீட்டில் இணைந்து இந்த யோகத்தை உருவாக்குகின்றன. இது மிகச்சிறந்த பகுப்பாய்வு திறனையும், கூர்மையான வணிக அறிவையும் தருகிறது. எழுத்து, தொழில்நுட்பம், மற்றும் ஆலோசனைத் துறைகளில் பெரும் வெற்றியைத் தரும்."
            })
        else:
            yogas.append({
                "Name": "Budhaditya Yoga",
                "Type": "Intellect & Commerce",
                "Description": f"The Sun and Mercury are structurally conjunct in your {p_houses.get('Sun')}th House. This forms a highly analytical and brilliant business mind. You combine the executive authority of the Sun with the tactical communication of Mercury, indicating strong wealth potential through advisory, technology, trade, or writing."
            })

    if "Jupiter" in p_pos and "Moon" in p_pos:
        jup_from_moon = (p_pos["Jupiter"] - p_pos["Moon"]) % 12 + 1
        if jup_from_moon in [1, 4, 7, 10]:
            if lang == "Tamil":
                yogas.append({
                    "Name": "கஜகேசரி யோகம் (Gajakesari Yoga)",
                    "Type": "புகழ் மற்றும் தெய்வீக பாதுகாப்பு",
                    "Description": "குரு உங்கள் சந்திரனுக்கு கேந்திரத்தில் இருப்பதால் இந்த மாபெரும் யோகம் உருவாகிறது. இது சமுதாயத்தில் பெரும் மதிப்பையும், தெய்வீக பாதுகாப்பையும், எதிரிகளை வெல்லும் சாதுரியத்தையும் தரும்."
                })
            else:
                yogas.append({
                    "Name": "Gajakesari Yoga",
                    "Type": "Fame & Institutional Protection",
                    "Description": "Jupiter is placed in a foundational angle from your Natal Moon. This is an elite combination for earning widespread respect and divine protection. It grants a noble reputation, social comfort, and the unique ability to defeat competitors through wisdom and diplomacy rather than brute force."
                })
    ...
    for p, y_name in pm_planets.items():
        if p in p_houses and p_houses[p] in [1, 4, 7, 10] and get_dignity(p, p_pos[p]) in ["Own", "Exalted"]:
            if lang == "Tamil":
                yogas.append({
                    "Name": f"{y_name} மகாபுருஷ யோகம்",
                    "Type": "தனித்துவமான ஆளுமை",
                    "Description": f"{t_p[p]} உங்கள் {p_houses[p]}-ஆம் வீட்டில் மிகவும் வலுவாக அமைந்திருப்பதால் இந்த யோகம் அமைகிறது. இது உங்களை ஒரு மாபெரும் தலைவராகவும் உங்கள் துறையில் அசைக்க முடியாத சக்தியாகவும் உயர்த்தும்."
                })
            else:
                yogas.append({
                    "Name": f"{y_name} Mahapurusha Yoga",
                    "Type": "Exceptional Domain Authority",
                    "Description": f"{p} is exceptionally strong in a foundational angle ({p_houses[p]}th House). You are mathematically destined to be a recognized authority in the domain ruled by {p}. This grants immense psychological resilience and elevates your status significantly."
                })
    ...
    if p_pos.get(lord_9) == p_pos.get(lord_10) and lord_9 != lord_10:
        if lang == "Tamil":
            yogas.append({
                "Name": "தர்ம கர்மாதிபதி யோகம்",
                "Type": "உயர்ந்த தொழில் அந்தஸ்து",
                "Description": "உங்களின் 9-ஆம் அதிபதியும் 10-ஆம் அதிபதியும் இணைந்திருப்பதால் இந்த யோகம் அமைகிறது. இது தொழில் ரீதியான மிக உயர்ந்த ராஜ யோகமாகும். நீங்கள் தொட்டதெல்லாம் துலங்கும்."
            })
        else:
            yogas.append({
                "Name": "Dharma Karmadhipati Yoga",
                "Type": "Ultimate Career Destiny",
                "Description": f"The rulers of your 9th House of Luck and 10th House of Career are united. This represents the highest form of professional Raja Yoga. Your internal life purpose and your external profession are seamlessly aligned."
            })

    if not yogas:
        if lang == "Tamil":
            yogas.append({
                "Name": "சுயமுயற்சி யோகம் (Independent Karma)",
                "Type": "சுயம்புவான வெற்றி",
                "Description": "உங்கள் ஜாதகம் எந்த ஒரு பாரம்பரிய யோகத்தையும் சார்ந்து இல்லை. உங்கள் வெற்றி முற்றிலும் உங்கள் சுயமுயற்சியாலும், விடாமுயற்சியாலும், புத்திக்கூர்மையாலும் மட்டுமே அமையும்."
            })
        else:
            yogas.append({
                "Name": "Independent Karma Yoga",
                "Type": "Self-Made Destiny",
                "Description": "Your chart does not rely on passive, inherited yogas. Instead, your success is generated purely through active free-will and executing the specific strategies highlighted in your House Scorecard."
            })
...
def analyze_education(p_pos, lagna_rasi, lang="English"):
    ...
    if lang == "Tamil":
        ...
        analysis.append(f"உங்கள் கல்வி மற்றும் அறிவாற்றலை 5-ஆம் அதிபதியான {t_p.get(lord_5, lord_5)} தீர்மானிக்கிறார். நீங்கள் எதையும் மேலோட்டமாக படிக்காமல், ஆழமாகப் புரிந்து கொள்ளும் குணம் கொண்டவர்.")
        if mercury_dig in ["Exalted", "Own"]:
            analysis.append("புதன் மிகவும் வலுவாக இருப்பதால், சிக்கலான தரவுகளைப் பகுப்பாய்வு செய்யும் அபார திறன் உங்களுக்கு உண்டு. கணக்கீடு, தொழில்நுட்பம் சார்ந்த துறைகளில் எளிதாக வெல்வீர்கள்.")
        elif mercury_dig == "Neecha":
            analysis.append("புதன் பலவீனமாக இருப்பதால், வெறும் மனப்பாடம் செய்வதை விட, உள்ளுணர்வு மற்றும் கற்பனைத்திறன் மூலம் நீங்கள் அதிகம் கற்கிறீர்கள். செயல்முறை கல்வியே உங்களுக்கு ஏற்றது.")
        else:
            analysis.append("உங்களின் தர்க்க அறிவும், கற்கும் திறனும் சீராக உள்ளது. தொடர்ச்சியான பயிற்சியின் மூலம் எந்த ஒரு துறையிலும் நீங்கள் சிறந்து விளங்க முடியும்.")
        ...
        if mercury_dig in ["Exalted", "Own"]:
            analysis.append(f"Because Mercury (the planet of logic) is highly dignified, your capacity to process complex data is elite. You excel in technical, analytical, or heavily communicative fields. You can out-study your peers easily.")
        elif mercury_dig == "Neecha":
            analysis.append("Your Mercury is mathematically debilitated, which actually means you possess highly intuitive, abstract intelligence rather than strict rote-memorization skills. Traditional classroom testing may frustrate you, but you excel in creative or big-picture problem-solving.")
        else:
            analysis.append("Your logical processing is balanced. You can apply yourself to a wide variety of subjects successfully, provided you maintain academic discipline and structured study routines.")
...
def analyze_health(p_pos, lagna_rasi, lang="English"):
    ...
    if lang == "Tamil":
        ...
        if ll_dig in ["Exalted", "Own"]:
            analysis.append(f"லக்னாதிபதி ({t_p.get(lagna_lord, lagna_lord)}) மிகவும் வலுவாக உள்ளார். இது உங்களுக்கு இரும்பு போன்ற உடல் வலிமையையும், வியக்கத்தக்க நோய் எதிர்ப்பு சக்தியையும் அளிக்கிறது. சோர்விலிருந்து மிக விரைவாக மீண்டு வருவீர்கள்.")
        elif ll_dig == "Neecha":
            analysis.append(f"லக்னாதிபதி ({t_p.get(lagna_lord, lagna_lord)}) பலவீனமாக உள்ளார். உங்கள் உடல் சக்தியை நீங்கள் மிகவும் கவனமாக கையாள வேண்டும். முறையான உணவு மற்றும் உறக்கமே உங்களுக்கு சிறந்த மருந்து.")
        else:
            analysis.append(f"லக்னாதிபதி ({t_p.get(lagna_lord, lagna_lord)}) சமநிலையில் உள்ளார். உங்களின் வாழ்க்கை முறை மற்றும் பழக்கவழக்கங்களே உங்கள் ஆரோக்கியத்தை தீர்மானிக்கும். நல்ல பழக்கங்கள் அதிக ஆற்றலைத் தரும்.")
        analysis.append("#### கவனிக்க வேண்டிய ஆரோக்கிய குறிப்புகள்")
        analysis.append(f"ஆரோக்கியத்தை குறிக்கும் 6-ஆம் அதிபதி {t_p.get(lord_6, lord_6)} ஆவார். இதைப் பொறுத்து நீங்கள் பின்வரும் விஷயங்களில் கவனம் செலுத்த வேண்டும்:")
        ...
        if ll_dig in ["Exalted", "Own"]:
            analysis.append(f"Your Ascendant Lord ({lagna_lord}) is exceptionally strong. This grants you a highly robust physical constitution and excellent natural immunity. You recover from illness and physical exhaustion much faster than average.")
        elif ll_dig == "Neecha":
            analysis.append(f"Your Ascendant Lord ({lagna_lord}) is weak by sign placement. Your physical energy is finite and must be carefully managed. You cannot rely on 'natural' vitality; you must strictly enforce dietary and sleep discipline to avoid chronic fatigue.")
        else:
            analysis.append(f"Your Ascendant Lord ({lagna_lord}) is in a neutral state. Your physical resilience is average. It will directly reflect your lifestyle choices—good routines yield high energy, while poor habits will immediately show physical consequences.")
...
def analyze_love_marriage(d1_lagna, d9_lagna, p_d9, p_d1, lang="English"):
    ...
    if lang == "Tamil":
        ...
        analysis.append(f"உங்கள் காதல் உணர்வுகள் 5-ஆம் அதிபதியான {t_p.get(lord_5, lord_5)} ஆல் ஆளப்படுகிறது. எனவே தொடக்கத்தில் உற்சாகமான மற்றும் உங்கள் எதிர்பார்ப்புகளுக்கு ஏற்ற உறவுகளை நாடுவீர்கள்.")
        analysis.append(f"ஆனால், உங்கள் நிரந்தர திருமண வாழ்க்கை நவாம்சத்தின் 7-ஆம் அதிபதியான {t_p.get(d9_7th_lord, d9_7th_lord)} இன் குணங்களைச் சார்ந்திருக்கும். இந்த குணங்களைக் கொண்ட துணையே உங்களுக்கு நீண்டகால மகிழ்ச்சியைத் தருவார்.")
        ...
        venus_dig = get_dignity("Venus", p_d9.get('Venus', 1))
        if venus_dig in ["Exalted", "Own"] or p_d1.get('Venus') == p_d9.get('Venus'): analysis.append("சுக்கிரன் மிகவும் வலுவாக உள்ளார். உங்கள் துணையிடம் ஆழமான அன்பைக் காட்டும் திறன் உங்களுக்கு இயல்பாகவே உள்ளது. திருமணத்திற்குப் பிறகு அதிர்ஷ்டம் கூடும்.")
        ...
        vargottama = [t_p.get(p, p) for p in p_d1.keys() if p != "Lagna" and p in p_d9 and p_d1[p] == p_d9[p]]
        if vargottama:
        ...
        venus_dig = get_dignity("Venus", p_d9.get('Venus', 1))
        if venus_dig in ["Exalted", "Own"] or p_d1.get('Venus') == p_d9.get('Venus'): analysis.append("Venus Strength: Exceptional. Your capacity to give and receive love matures beautifully over time. Post-marriage, your financial luck, social status, and general life fortune will see a marked, structural increase.")
        ...
        if vargottama:
            ...
            analysis.append("Your planetary energies are highly adaptable. You evolve and dynamically change your approach to challenges as you grow older.")
    return analysis


def analyze_career_professional(p_pos, d10_lagna, lagna_rasi, sav_scores, bhava_placements, lang="English"):
    analysis = []
    sun_rasi_h = (p_pos['Sun'] - lagna_rasi) % 12 + 1
    sun_bhava_h = bhava_placements.get('Sun', sun_rasi_h)
    ...
    if lang == "Tamil":
        ...
        analysis.append(f"உங்கள் தசாம்ச (D10) அதிபதி {t_p.get(d10_lord, d10_lord)}. உங்களுக்குப் பொருத்தமான தொழில் முறை: **{role}**. உங்களின் மிகப்பெரிய பலம் '**{traits}**' ஆகும். நெருக்கடியான நேரங்களில் இதை முழுமையாகப் பயன்படுத்துங்கள்.")
    else:
        ...
        if sun_rasi_h != sun_bhava_h:
            ...
            analysis.append(f"Direct Impact: Your Sun aligns perfectly in Sign and House ({sun_rasi_h}th). Your internal intent perfectly matches your external career results.")
...
def get_transit_positions(f_year):
    ...
    return {
        "Saturn": int(swe.calc_ut(jd, swe.SATURN, swe.FLG_SIDEREAL)[0][0] / 30) + 1,
        "Jupiter": int(swe.calc_ut(jd, swe.JUPITER, swe.FLG_SIDEREAL)[0][0] / 30) + 1,
        "Rahu": int(swe.calc_ut(jd, swe.MEAN_NODE, swe.FLG_SIDEREAL)[0][0] / 30) + 1
    }
...
def get_next_transit_date(planet_id, current_rasi, start_date):
    ...
    for _ in range(1200):
        ...
        if new_rasi != current_rasi:
            return search_date.strftime("%d %b %Y"), new_rasi
...
def get_micro_transits(f_year, p_lon_absolute, lang="English"):
    ...
    for step in range(0, 365, 5):
        ...
        for trp, tid in tr_planets.items():
            ...
            for np_name in nat_planets:
                n_lon = p_lon_absolute.get(np_name, 0)
                diff = abs(tr_lon - n_lon)
                if diff > 180:
                    diff = 360 - diff
                if diff <= 2.5:
                    key = (trp, np_name)
                    if key not in active_conjunctions:
                        active_conjunctions[key] = []
                    active_conjunctions[key].append(current_date)

    for (trp, np_name), dates in active_conjunctions.items():
        if not dates:
            continue
        ...
        meaning = ""
        if lang == "Tamil":
            t_trp = t_p.get(trp, trp)
            t_np = t_p.get(np_name, "லக்னம்")
            trigger_txt = f"கோச்சார {t_trp}, ஜனன {t_np} மீது இணைகிறது"
            if trp == "Saturn":
                meanings_ta = {
                    "Sun": "தொழில் மற்றும் அதிகாரத்தில் மன அழுத்தம் கூடும். மேலதிகாரிகளை அனுசரித்துச் செல்லவும்.",
                    "Moon": "ஏழரைச் சனியின் உச்சம். உணர்ச்சிகளைக் கட்டுப்படுத்தி, மன அமைதியை பேணுவது அவசியம்.",
                    "Mars": "வேகம் விவேகமல்ல. பயணங்களில் மிகவும் கவனம் தேவை. கோபத்தை தவிர்க்கவும்.",
                    "Mercury": "சிந்தனை ஒருமுகப்படும். கடினமான வேலைகளை முடிக்க சிறந்த நேரம்.",
                    "Jupiter": "வளர்ச்சிக்கும் கட்டுப்பாட்டிற்கும் இடையிலான போராட்டம். நிதியை கவனமாகக் கையாளவும்.",
                    "Venus": "உறவுகளில் உண்மை நிலை புரியும். ஆடம்பர செலவுகளைத் தவிர்க்கவும்.",
                    "Saturn": "சனி ஆவர்த்தனம். வாழ்க்கை முறையில் ஒரு மாபெரும் கட்டமைப்பு மாற்றம் நிகழும்.",
                    "Lagna": "உடல் சோர்வு ஏற்படும். உங்கள் பொறுப்புகள் பலமடங்கு அதிகரிக்கும்."
                }
                meaning = meanings_ta.get(np_name, "")
            elif trp == "Jupiter":
                meanings_ta = {
                    "Sun": "பதவி உயர்வு மற்றும் சமூகத்தில் மாபெரும் அந்தஸ்து கிடைக்கும்.",
                    "Moon": "மனதில் அமைதி நிலவும். குடும்பத்தில் சுபகாரியங்கள் நடைபெறும்.",
                    "Mars": "தைரியம் கூடும். புதிய முயற்சிகளைத் தொடங்க மிகச் சிறந்த தருணம்.",
                    "Mercury": "அறிவாற்றல் பெருகும். வியாபாரம் மற்றும் கல்வியில் மாபெரும் வெற்றி.",
                    "Jupiter": "குரு ஆவர்த்தனம். 12 வருடங்களுக்கு ஒருமுறை வரும் பொன்னான அதிர்ஷ்ட காலம்.",
                    "Venus": "பொருளாதார ஏற்றம் மற்றும் குடும்பத்தில் மகிழ்ச்சி பொங்கும் நேரம்.",
                    "Saturn": "நீண்டகாலமாக இருந்த தடைகள் நீங்கி, உங்கள் உழைப்பிற்கு ஏற்ற பலன் கிடைக்கும்.",
                    "Lagna": "தெய்வீக அருள் உங்களை பாதுகாக்கும். முகத்தில் தேஜஸ் அதிகரிக்கும்."
                }
                meaning = meanings_ta.get(np_name, "")
            elif trp == "Rahu":
                meanings_ta = {
                    "Sun": "அதிகாரம் மீதான ஆசை அதிகரிக்கும். மாயைகளில் சிக்க வேண்டாம்.",
                    "Moon": "மனதில் குழப்பங்கள் வரலாம். தியானம் செய்வது மிகவும் அவசியம்.",
                    "Mars": "கட்டுக்கடங்காத ஆற்றல் உருவாகும். விபத்துகள் குறித்து எச்சரிக்கை தேவை.",
                    "Mercury": "தொழில்நுட்ப அறிவில் ஈடுபாடு கூடும். ஏமாற்று வேலைகளில் கவனமாக இருக்கவும்.",
                    "Jupiter": "பாரம்பரிய விதிகளை மீறி வெற்றி பெற நினைப்பீர்கள். திடீர் பணவரவு உண்டு.",
                    "Venus": "ஆடம்பரம் மற்றும் சிற்றின்ப ஆசைகள் அதிகரிக்கும். கட்டுப்பாட்டுடன் இருக்கவும்.",
                    "Saturn": "வாழ்க்கை வேகமாக மாறும். சற்று மன அழுத்தம் இருந்தாலும் வெற்றி நிச்சயம்.",
                    "Lagna": "வாழ்க்கைப் பாதையை முற்றிலும் மாற்றிக்கொள்ளும் எண்ணம் மேலோங்கும்."
                }
                meaning = meanings_ta.get(np_name, "")
        else:
            trigger_txt = f"Transiting {trp} crosses Natal {np_name}"
            if trp == "Saturn":
                meanings_en = {
                    "Sun": "Heavy pressure on career and ego. Yield to authority or carry heavy professional burdens.",
                    "Moon": "Peak Sade Sati energy. Emotional weight, reality checks, and forced maturity.",
                    "Mars": "Extreme frustration or blocked energy. Avoid physical risks or aggressive confrontations.",
                    "Mercury": "Serious mental focus. Great for heavy analytical work, but restrictive for lighthearted communication.",
                    "Jupiter": "A clash between growth and restriction. Financial structures must be solidified and secured.",
                    "Venus": "Relationships face a reality check. Frivolous spending is punished; commitment is strictly tested.",
                    "Saturn": "Saturn Return. A major milestone of completely rebuilding your life structure from the ground up.",
                    "Lagna": "Massive personal restructuring. High physical fatigue. You are stepping into a higher level of maturity."
                }
                meaning = meanings_en.get(np_name, "")
            elif trp == "Jupiter":
                meanings_en = {
                    "Sun": "Massive visibility and career grace. Promotions, favor from bosses, and leadership opportunities arise.",
                    "Moon": "Deep emotional healing. Auspicious events at home, property gains, or family expansion.",
                    "Mars": "A surge of confident energy. Excellent time to launch bold initiatives or legal actions.",
                    "Mercury": "Intellectual breakthroughs. High success in trade, writing, deals, and networking.",
                    "Jupiter": "Jupiter Return. A 12-year peak of luck, spiritual alignment, and financial opportunity.",
                    "Venus": "High romantic and financial luck. A period of luxury, celebrations, and ease in relationships.",
                    "Saturn": "Relief from long-standing burdens. Your hard work finally gets recognized and rewarded.",
                    "Lagna": "Physical and spiritual protection. A highly optimistic period where your personal aura shines."
                }
                meaning = meanings_en.get(np_name, "")
            elif trp == "Rahu":
                meanings_en = {
                    "Sun": "Sudden, almost obsessive desire for power. Beware of ego-traps or clashes with male authority.",
                    "Moon": "High emotional turbulence or anxiety. Unconventional desires. Guard your mental peace carefully.",
                    "Mars": "Explosive, unpredictable energy. Massive drive, but high risk of accidents or impulsive anger.",
                    "Mercury": "Obsessive thinking. Good for tech/coding, but beware of deceptive communications or scams.",
                    "Jupiter": "Breaking traditional rules for success. Financial windfalls through unorthodox means.",
                    "Venus": "Intense romantic or financial desires. Sudden infatuations or luxurious spending binges.",
                    "Saturn": "Karmic acceleration. Breaking old rules to build new structures. Stressful but highly productive.",
                    "Lagna": "A sudden urge to completely reinvent your physical appearance or life path. Restless energy."
                }
                meaning = meanings_en.get(np_name, "")

        if meaning:
            events.append({"Trigger": trigger_txt, "Dates": date_txt, "Impact": meaning})
    return events
...
def generate_mahadasha_table(moon_lon, birth_date, lang="English"):
    ...
    bal = 1 - ((moon_lon % NAK_SIZE) / NAK_SIZE)
    ...
    timeline = [{
        "Age (From-To)": f"0 - {int((first_end - birth_date).days / 365.25)}",
        "Years": f"{curr_date.year} - {first_end.year}",
        "Mahadasha": t_p.get(first_lord, first_lord) if lang == "Tamil" else first_lord,
        "Prediction": preds.get(first_lord, "")
    }]
    curr_date = first_end
    for i in range(1, 9):
        ...
        timeline.append({
            "Age (From-To)": f"{int((curr_date - birth_date).days / 365.25)} - {int((end_date - birth_date).days / 365.25)}",
            "Years": f"{curr_date.year} - {end_date.year}",
            "Mahadasha": t_p.get(lord, lord) if lang == "Tamil" else lord,
            "Prediction": preds.get(lord, "")
        })
        curr_date = end_date
    return timeline
...
def get_detailed_bhukti_analysis(md, ad, planet_bhava_map, lang="English"):
    ...
    if lang == "Tamil":
        ...
        t_topics = {
            1: "சுய அடையாளம், உடல் ஆரோக்கியம் மற்றும் புதிய தொடக்கங்கள்",
            2: "செல்வம், குடும்பம் மற்றும் வருமானப் பெருக்கம்",
            3: "தைரியம், குறுகிய பயணங்கள் மற்றும் தகவல் தொடர்பு",
            4: "வீடு, வாகனம், தாயார் மற்றும் மன அமைதி",
            5: "குழந்தைகள், கலை, மற்றும் புத்திசாலித்தனம்",
            6: "ஆரோக்கியம், கடன் தீர்வு மற்றும் எதிரிகளை வெல்லுதல்",
            7: "திருமணம், கூட்டாண்மை மற்றும் சமுதாய உறவுகள்",
            8: "திடீர் மாற்றங்கள், ரகசியங்கள் மற்றும் ஆழமான தேடல்",
            9: "பாக்கியம், தந்தை, உயர் கல்வி மற்றும் ஆன்மீகம்",
            10: "தொழில் வெற்றி, பதவி உயர்வு மற்றும் சமூக அந்தஸ்து",
            11: "லாபம், நெட்வொர்க் மற்றும் ஆசைகள் நிறைவேறுதல்",
            12: "பயணங்கள், முதலீடுகள், செலவுகள் மற்றும் ஆன்மீக தேடல்"
        }
        t_md = t_p.get(md, md)
        t_ad = t_p.get(ad, ad)
        ...
        if md == ad:
            base += f"உங்கள் ஜாதகத்தில் {t_md} {md_house}-ஆம் வீட்டில் அமர்ந்துள்ளதால், இந்த காலகட்டம் முற்றிலும் '{t_topics.get(md_house, '')}' என்பதை சுற்றியே அமையும். இது ஒரு தசா சந்தி (அதிகபட்ச தாக்கம்) என்பதால் இதன் சக்தி வீரியமாக இருக்கும்.\n\n"
        else:
            base += f"உங்கள் ஜாதகத்தில் {t_md} {md_house}-ஆம் வீட்டில் அமர்ந்துள்ளதால் நீண்டகால இலக்குகள் அதைச் சார்ந்திருக்கும். ஆனால் தற்போது {t_ad} புக்தி நடப்பதால், உங்கள் அன்றாட நிகழ்வுகள் மற்றும் பலன்கள் நேரடியாக '{t_topics.get(ad_house, '')}' வழியே நடைபெறும்.\n\n"
        base += "முக்கிய கணிப்புகள்:\n"
        base += f"- உங்களின் முழு கவனமும் '{t_topics.get(ad_house, '').split(',')[0]}' மீது திரும்பும்.\n"
        if ad in ["Saturn", "Mars", "Rahu", "Ketu", "Sun"]:
            base += "எச்சரிக்கை: இது இயற்கையாகவே சற்று தீவிரமான கிரகத்தின் காலம் என்பதால், கோபத்தையும் அவசர முடிவுகளையும் கட்டாயம் தவிர்க்க வேண்டும். ஆவணங்களில் கையெழுத்திடும் முன் கவனமாகப் படிக்கவும்.\n"
        else:
            base += "எச்சரிக்கை: இது ஒரு சாதகமான காலம் என்றாலும், சோம்பேறித்தனத்தை தவிர்த்து தொடர்ந்து உழைப்பது அவசியம். உங்கள் தினசரி கட்டுப்பாடுகளைத் தளர்த்த வேண்டாம்.\n"
        ...
        topics = {
            1: "personal identity, physical vitality, and major life directions",
            2: "wealth accumulation, family dynamics, and financial planning",
            3: "courage, self-effort, short travels, and communication",
            4: "domestic peace, real estate, mother, and inner emotional foundations",
            5: "creativity, children, speculative investments, and intellect",
            6: "health routines, resolving debts, and overcoming competitors",
            7: "marriage, business partnerships, and public dealings",
            8: "deep transformation, hidden knowledge, unexpected events, and shared finances",
            9: "luck, higher learning, long-distance travel, and mentorship",
            10: "career advancement, public status, and professional authority",
            11: "network expansion, large gains, and the fulfillment of long-term desires",
        ...
        if md == ad:
            base += f"Because {md} is placed in your {md_house}th House, this period is intensely, heavily focused. The absolute center of your life right now revolves around {topics.get(md_house, '')}. This is a 'Dasha Sandhi' (peak intensification) where the planetary energy is completely undiluted.\n\n"
        else:
            base += f"In your specific chart, {md} sits in the {md_house}th House, making your long-term, background focus center on {topics.get(md_house, '')}. However, {ad} is currently activating your {ad_house}th House. This means your immediate, day-to-day events, actions, and results will manifest specifically through {topics.get(ad_house, '')}.\n\n"
        base += "Key Predictions:\n"
        base += f"- Core focus shifts heavily toward {topics.get(ad_house, '').split(',')[0]}.\n"
        if ad in ["Saturn", "Mars", "Rahu", "Ketu", "Sun"]:
            base += "Precautions: This sub-period is ruled by a naturally intense, aggressive planet. You must actively avoid impulsive decisions, practice extreme patience, and do not force outcomes prematurely. Verify all legal documents carefully.\n"
        else:
            base += "Precautions: While this is a generally supportive and gentle energy, do not become intellectually or physically complacent. Avoid over-indulgence and heavily maintain your daily discipline.\n"
...
def generate_current_next_bhukti(moon_lon, birth_date, planet_bhava_map, lang="English"):
                ...
                if ad_start <= current_date <= ad_end:
                    ...
                    for j in range(9):
                        ...
                        if pd_start <= current_date <= pd_end:
                            if lang == "Tamil":
                                t_md = t_p.get(md_lord, md_lord)
                                t_ad = t_p.get(ad_lord, ad_lord)
                                t_pd = t_p.get(pd_lord, pd_lord)
                            else:
                                t_md, t_ad, t_pd = md_lord, ad_lord, pd_lord

                            active_pd = {
                                "MD": t_md, "AD": t_ad, "PD": t_pd,
                                "Start": pd_start.strftime('%d %b %Y'),
                                "End": pd_end.strftime('%d %b %Y')
                            }
                            ...
                            p1 = {
                                "Type": lbl_curr,
                                "Phase": f"{t_md} - {t_ad}",
                                "Dates": f"{ad_start.strftime('%b %Y')} to {ad_end.strftime('%b %Y')}",
                                "Text": get_detailed_bhukti_analysis(md_lord, ad_lord, planet_bhava_map, lang)
                            }
                            next_ad = DASHA_ORDER[(ad_idx + i + 1) % 9]
                            t_next_ad = t_p.get(next_ad, next_ad) if lang == "Tamil" else next_ad
                            p2 = {
                                "Type": lbl_next,
                                "Phase": f"{t_md} - {t_next_ad}",
                                "Dates": "விரைவில்..." if lang == "Tamil" else "Upcoming",
                                "Text": get_detailed_bhukti_analysis(md_lord, next_ad, planet_bhava_map, lang)
                            }
    ...
    # Fallback: return empty phases if no match found (prevents None unpack crash)
    fallback_label = "தகவல் கிடைக்கவில்லை" if lang == "Tamil" else "No Active Phase Found"
    return [
        {"Type": fallback_label, "Phase": "-", "Dates": "-", "Text": "-"},
        {"Type": fallback_label, "Phase": "-", "Dates": "-", "Text": "-"}
    ], {"MD": "-", "AD": "-", "PD": "-", "Start": "-", "End": "-"}
...
def get_south_indian_chart_html(p_pos, lagna_rasi, title, lang="English"):
    ...
    # Unified zodiac label helper (handles both dict and list)
    def get_z(idx):
        z = ZODIAC_TA if lang == "Tamil" else ZODIAC
        if isinstance(z, dict):
            return z.get(idx, "")
        # list: ZODIAC is assumed 1-indexed, so subtract 1
        try:
            return z[idx - 1]
    ...
    return (
        f"<div style='max-width: 450px; margin: auto; font-family: sans-serif;'>"
        f"<table style='width: 100%; border-collapse: collapse; text-align: center; font-size: 14px; background-color: #ffffff; border: 2px solid #333;'>"
        f"<tr>"
        f"<td style='border: 1px solid #333; width: 25%; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(12)} (12)</div>{g[12]}</td>"
        f"<td style='border: 1px solid #333; width: 25%; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(1)} (1)</div>{g[1]}</td>"
        f"<td style='border: 1px solid #333; width: 25%; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(2)} (2)</div>{g[2]}</td>"
        f"<td style='border: 1px solid #333; width: 25%; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(3)} (3)</div>{g[3]}</td>"
        f"</tr><tr>"
        f"<td style='border: 1px solid #333; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(11)} (11)</div>{g[11]}</td>"
        f"<td colspan='2' rowspan='2' style='border: none; vertical-align: middle; font-weight: bold; font-size: 16px; color:#2c3e50; background-color: #ffffff;'>{title}</td>"
        f"<td style='border: 1px solid #333; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(4)} (4)</div>{g[4]}</td>"
        f"</tr><tr>"
        f"<td style='border: 1px solid #333; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(10)} (10)</div>{g[10]}</td>"
        f"<td style='border: 1px solid #333; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(5)} (5)</div>{g[5]}</td>"
        f"</tr><tr>"
        f"<td style='border: 1px solid #333; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(9)} (9)</div>{g[9]}</td>"
        f"<td style='border: 1px solid #333; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(8)} (8)</div>{g[8]}</td>"
        f"<td style='border: 1px solid #333; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(7)} (7)</div>{g[7]}</td>"
        f"<td style='border: 1px solid #333; height: 95px; vertical-align: top; padding: 5px; background-color:#fafafa;'><div style='font-size:11px; color:#7f8c8d; text-align:left;'>{get_z(6)} (6)</div>{g[6]}</td>"
        f"</tr></table></div>"
    )
...
# ==========================================
def generate_html_report(name_in, p_pos, p_d9, lagna_rasi, moon_rasi, sav_scores, career_txt, edu_txt, health_txt, love_txt, id_data, lagna_str, moon_str, star_str, yogas, fc, micro_transits, mahadasha_data, phases, pd_info, guide, transit_texts, lang="English"):
    """moon_rasi is now an explicit parameter (was previously missing, causing NameError)."""
    ...
    d9_lagna_idx = get_navamsa_chart(p_pos.get("Lagna", lagna_rasi * 30) if isinstance(p_pos.get("Lagna"), float) else lagna_rasi * 30)
    ...
    l_str = get_z_name(lagna_rasi)
    m_str = get_z_name(moon_rasi)
    ...
    <body>
        ...
        <p><b>{"பலங்கள்" if lang=="Tamil" else "Strengths"}:</b> {id_data.get('Strengths', '')}</p>
        <p><b>{"பலவீனங்கள்" if lang=="Tamil" else "Weaknesses"}:</b> {id_data.get('Weaknesses', '')}</p>
    ...
    for y in yogas:
        html += f"<h4>{y['Name']} ({y['Type']})</h4><p>{y['Description']}</p>"
    ...
    html += f"<div class='remedy-box'><ul><li><b>{d_lbl}:</b> {guide.get('Deity', '')}</li><li><b>{m_lbl2}:</b> {guide.get('Mantra', '')}</li><li><b>{h_lbl}:</b> {guide.get('Daily', '')}</li><li><b>{b_lbl}:</b> {guide.get('Benefit', '')}</li><li><b>{a_lbl}:</b> {guide.get('Accessory', '')}</li><li><b>{av_lbl}:</b> {guide.get('Avoid', '')}</li></ul></div>"
...
# ==========================================
# 6. STREAMLIT APP UI & EXECUTION (Single, clean sidebar)
# ==========================================
...
with st.sidebar:
    ...
    if calc_btn:
        if not name_in or not city:
            st.error("Please enter a Name and City!")
        else:
            st.session_state.report_generated = True


# ==========================================
# 7. EXECUTION BLOCK (Single, clean block)
# ==========================================
if st.session_state.report_generated:
    with st.spinner("Calculating exact astronomical data..." if LANG == "English" else "ஜாதகம் கணிக்கப்படுகிறது..."):
        ...
        # Add Ketu to absolute positions for micro-transit analysis
        ...
        guide = db_life.get(RASI_RULERS[moon_rasi - 1], db_life.get("Moon", {}))

        # Lagna sign key for identity db (using ZODIAC English name)
        lagna_en_key = ZODIAC[lagna_rasi] if isinstance(ZODIAC, dict) else ZODIAC[lagna_rasi - 1]
        report_id_data = db_id.get(lagna_en_key, list(db_id.values())[0])

    # --- UI LAYOUT ---
    c_left, c_right = st.columns([3, 1])
    with c_left:
        st.subheader(f"Analysis for {name_in}" if LANG == "English" else f"ஜோதிட அறிக்கை: {name_in}")
        l_name = _z(lagna_rasi)
        m_name = _z(moon_rasi)
    ...
    with c_right:
        # moon_rasi is now passed explicitly to fix the NameError
        html_bytes = generate_html_report(
            name_in, p_pos, p_d9, lagna_rasi, moon_rasi, sav_scores,
            career_txt, edu_txt, health_txt, love_txt,
            report_id_data, lagna_en_key, _z(moon_rasi), nak,
            yogas, fc, micro_transits, mahadasha_data, phases, pd_info,
            guide, transit_texts, lang=LANG
        )
        st.download_button(
            label="📄 Download Full HTML Report",
            data=html_bytes,
            file_name=f"{name_in}_Astro_Report.html",
            mime="text/html"
        )

    tb_lbls = (
        ["Profile", "Scorecard", "Work & Intellect", "Love & Health", "Yogas", "Forecast", "Roadmap", "💬 AI Oracle"]
        if LANG == "English" else
        ["சுயவிவரம்", "அஷ்டகவர்க்கம்", "கல்வி & தொழில்", "திருமணம் & ஆரோக்கியம்", "யோகங்கள்", "ஆண்டு பலன்கள்", "தசா புக்தி", "💬 AI ஜோதிடர்"]
    )
    ...
    with t1:
        ...
        st.markdown(f"**{'நோக்கம்' if LANG=='Tamil' else 'Purpose'}:** {user_id.get('Purpose', '')}")
        st.markdown(f"**{'குணம்' if LANG=='Tamil' else 'Personality'}:** {user_id.get('Personality', '')}")
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f"#### {'பலங்கள்' if LANG=='Tamil' else 'Strengths'}")
            st.markdown(user_id.get('Strengths', ''))
        with c2:
            st.markdown(f"#### {'பலவீனங்கள்' if LANG=='Tamil' else 'Weaknesses'}")
            st.markdown(user_id.get('Weaknesses', ''))
        st.divider()
        ...
        table_md = (
            f"<table style='width: 80%; margin: 20px auto; border-collapse: collapse; font-family: sans-serif; font-size: 15px; text-align: center;'>"
            f"<tr style='background-color: #f8f9fa; border-bottom: 2px solid #ccc;'>"
            f"<th style='padding: 12px 8px;'>{headers[0]}</th>"
            f"<th style='padding: 12px 8px;'>{headers[1]}</th>"
            f"<th style='padding: 12px 8px;'>{headers[2]}</th>"
            f"<th style='padding: 12px 8px;'>{headers[3]}</th>"
            f"<th style='padding: 12px 8px;'>{headers[4]}</th></tr>"
        )
        for row in master_table:
            p_name = TAMIL_NAMES.get(row['Planet'], row['Planet']) if LANG == "Tamil" else row['Planet']
            table_md += (
                f"<tr style='border-bottom: 1px solid #eee;'>"
                f"<td style='padding: 12px 8px;'><b>{p_name}</b></td>"
                f"<td style='padding: 12px 8px;'>{row['Rasi']}</td>"
                f"<td style='padding: 12px 8px;'>{row['House']}</td>"
                f"<td style='padding: 12px 8px;'>{row['Dignity']}</td>"
                f"<td style='padding: 12px 8px;'>{row['Status']}</td></tr>"
            )
        table_md += "</table>"
    ...
    with t2:
        ...
        fig_bar.add_vline(
            x=28, line_width=2, line_dash="dash", line_color="#7f8c8d",
            annotation_text="Average (28)" if LANG == "English" else "சராசரி (28)",
        ...
        with c1:
            ...
            for s, h in sorted_houses[:3]:
                st.markdown(f"**{p_lbl} {h} - {s} Points:**\n{db_lev.get(h, '')}")
        with c2:
    ...
    with t7:
        ...
        planet_colors = {
            ...
            "சூரியன்": "#d35400", "சந்திரன்": "#95a5a6", "செவ்வாய்": "#c0392b",
            "புதன்": "#27ae60", "குரு": "#f39c12", "சுக்கிரன்": "#8e44ad",
            "சனி": "#2c3e50", "ராகு": "#34495e", "கேது": "#7f8c8d"
        }
        ...
        fig_timeline.add_trace(go.Bar(
            y=[''] * len(dasha_names), x=durations, base=start_years,
            name="Mahadashas", orientation='h',
            text=dasha_names, textposition='inside', textangle=0,
        ...
        md_table_html = (
            f"<table style='width: 100%; border-collapse: collapse; font-family: sans-serif; font-size: 14px; margin-top: 20px;'>"
            f"<tr style='border-bottom: 2px solid #ddd; background-color: #fdfdfd;'>"
            f"<th style='padding: 10px 8px; text-align: left; width: 10%;'>{h_age}</th>"
            f"<th style='padding: 10px 8px; text-align: left; width: 10%;'>{h_yrs}</th>"
            f"<th style='padding: 10px 8px; text-align: left; width: 15%;'>{h_md}</th>"
            f"<th style='padding: 10px 8px; text-align: left; width: 65%;'>{h_pred}</th></tr>"
        )
        for row in mahadasha_data:
            s_year, e_year = row['Years'].split(' - ')
            md_table_html += (
                f"<tr style='border-bottom: 1px solid #eee;'>"
                f"<td style='padding: 10px 8px; vertical-align: top;'>{row['Age (From-To)']}</td>"
                f"<td style='padding: 10px 8px; vertical-align: top;'>{s_year}<br>{e_year}</td>"
                f"<td style='padding: 10px 8px; vertical-align: top;'><b>{row['Mahadasha']}</b></td>"
                f"<td style='padding: 10px 8px; vertical-align: top;'>{row['Prediction']}</td></tr>"
            )
        md_table_html += "</table>"
    ...
    with t8:
        ...
        if prompt_input := st.chat_input("Ask a question... / உங்கள் கேள்வியைக் கேளுங்கள்..."):
            if not GEMINI_API_KEY:
                st.error("API Key missing! Please add your key to Streamlit secrets or api_config.py")
            else:
                    ...
                    with st.chat_message("assistant"):
                        with st.spinner("Analyzing..." if LANG == "English" else "கணிக்கப்படுகிறது..."):
                            try:
                                ...
                                chart_context = f"User is Ascendant {lagna_en_key} and Moon {_z(moon_rasi)}. "
                                chart_context += f"Planetary positions: {p_pos}. "
                                if pd_info and pd_info.get("MD", "-") != "-":
                                    chart_context += f"Current Dasha Phase: {pd_info['MD']} Mahadasha, {pd_info['AD']} Antardasha. "
                                ...
                                chart_context += f"For {f_year}, major transits are: Saturn in {_z(t_data['Saturn']['Rasi'])}, Jupiter in {_z(t_data['Jupiter']['Rasi'])}, Rahu in {_z(t_data['Rahu']['Rasi'])}. "
